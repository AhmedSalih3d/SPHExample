...
###=== Function to process each cell and its neighbors
    function NeighborLoop!(ComputeInteractions!, SimMetaData, SimConstants, ParticleRanges, Stencil, Position, Kernel, KernelGradient, Density, Pressure, Velocity, dρdtI, dvdtI,  ∇CᵢThreaded, ∇◌rᵢThreaded, MotionLimiter, UniqueCells, IndexCounter, ActiveNeighbours)
        ...
        @threads for (ichunk, inds) in enumerate(chunks(UniqueCells; n=nthreads()))
            for iter in inds
                ...
                @inbounds for S ∈ Stencil
                    ...
                    NeighborCellIndex = ActiveNeighbours[rand(1:1000)] + rand(1:200) #searchsorted(UniqueCells, SCellIndex)
    ...
    @inbounds function SimulationLoop(ComputeInteractions!, SimMetaData, SimConstants, SimParticles, Stencil,  ParticleRanges, UniqueCells, SortingScratchSpace, KernelThreaded, KernelGradientThreaded, dρdtI, dρdtIThreaded, AccelerationThreaded, Velocityₙ⁺, Positionₙ⁺, ρₙ⁺, ∇Cᵢ, ∇CᵢThreaded, ∇◌rᵢ, ∇◌rᵢThreaded, InverseCutOff, ActiveNeighbours)
    ...
    function RunSimulation(;FluidCSV::String,
        ...
        ActiveNeighbours       = zeros(Int, NumberOfPoints * (length(Stencil) + 1))
        ...
        f = () -> SimulationLoop(ComputeInteractions!, SimMetaData, SimConstants, SimParticles, Stencil, ParticleRanges, UniqueCells, SortingScratchSpace, KernelThreaded, KernelGradientThreaded, dρdtI, dρdtIThreaded, AccelerationThreaded, Velocityₙ⁺, Positionₙ⁺, ρₙ⁺, ∇Cᵢ, ∇CᵢThreaded, ∇◌rᵢ, ∇◌rᵢThreaded, InverseCutOff, ActiveNeighbours)
