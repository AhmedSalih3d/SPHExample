...
using Base.Threads
    ...
    function UpdateNeighbors!(Particles, Stencil, InverseCutOff, SortingScratchSpace, ParticleRanges, UniqueCells, NeighborListVector, IndexStarts)
...
###=== Function to process each cell and its neighbors
    ...
    @inbounds function SimulationLoop(ComputeInteractions!, SimMetaData, SimConstants, SimParticles, Stencil,  ParticleRanges, UniqueCells, NeighborListVector, IndexStarts, SortingScratchSpace, dρdtI, Velocityₙ⁺, Positionₙ⁺, ρₙ⁺, ∇Cᵢ, ∇◌rᵢ, MotionDefinition, InverseCutOff)
        ...
        if mod(SimMetaData.Iteration, ceil(Int, 1 / (SimConstants.c₀ * dt * (1/SimConstants.CFL)) )) == 0 || SimMetaData.Iteration == 1
            @timeit SimMetaData.HourGlass "02 Calculate IndexCounter" IndexCounter = UpdateNeighbors!(SimParticles, Stencil, InverseCutOff, SortingScratchSpace,  ParticleRanges, UniqueCells, NeighborListVector, IndexStarts)
        else
    ...
    function RunSimulation(;SimGeometry::Dict, #Don't further specify type for now
        ...
        @inbounds while true
            ...
            SimulationLoop(ComputeInteractions!, SimMetaData, SimConstants, SimParticles, Stencil, ParticleRanges, UniqueCells, NeighborListVector, IndexStarts, SortingScratchSpace, dρdtI, Velocityₙ⁺, Positionₙ⁺, ρₙ⁺, ∇Cᵢ, ∇◌rᵢ, MotionDefinition, InverseCutOff)
