...
using TimerOutputs: @timeit
    ...
    function NeighborLoopPerParticle!(SimDensityDiffusion::SDD, SimViscosity::SV, SimKernel,
        ...
        @inbounds @threads for i in eachindex(Position)
            ...
            NeighborCellStart = Int(NeighborCellListOffsets[CellListIndex])
            NeighborCellEnd = Int(NeighborCellListOffsets[CellListIndex + 1]) - 1
            ...
            for NeighborCellCursor in NeighborCellStart:NeighborCellEnd
                NeighborIdx = Int(NeighborCellListIndices[NeighborCellCursor])
    ...
    function NeighborLoopMDBC!(SimKernel,
                               ...
                               CellIndexMap::Dict{CellType, IndexType}, SimParticles, bᵧ, Aᵧ) where {Dimensions, FloatType, SMode, KMode, BMode, LMode, CellType, IndexType<:Integer}
        ...
        @inbounds @threads for iter in eachindex(GhostPoints)
            ...
            if !iszero(GhostPoint)
                ...
                @inbounds for Offset ∈ FullStencil
                    NeighborIdxValue = get(CellIndexMap, CellIndex + Offset, zero(IndexType))
                    if iszero(NeighborIdxValue)
                        continue
                    end
                    NeighborIdx = Int(NeighborIdxValue)
