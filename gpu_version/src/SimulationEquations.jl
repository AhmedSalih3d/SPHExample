...
# Magic constants for the initial guess of x^(1/7): roughly (6/7) of the
# exponent bias expressed in the bit pattern of the float type.
@inline _root7_magic(::Type{Float64}) = 0x36cd000000000000
@inline _root7_magic(::Type{Float32}) = 0x36680000

@inline function Estimate7thRoot(x::T) where {T <: Union{Float32, Float64}}
    U = Base.uinttype(T)
    # initial guess based on fast inverse sqrt trick but adjusted to compute x^(1/7)
    t = copysign(reinterpret(T, _root7_magic(T) + reinterpret(U, abs(x)) ÷ U(7)), x)
