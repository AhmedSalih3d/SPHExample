...
function create_data_array_element(name::String, data::AbstractVector{T}, offset::Int) where T
    ...
    dataarray = XML.Element("DataArray")
...
function ConvertHDFtoVTP(filename::String, DictVariable)
    ...
    xml_doc =  XML.Document(XML.Declaration(version=1.0,encoding="utf-8"))
    vtk_file = XML.Element("VTKFile")
    ...
    polydata  = XML.Element("PolyData")
    piece     = XML.Element("Piece")
    N = length(points)
    ...
    pointdata  = XML.Element("PointData")
...
let
    ...
    for iter in dict_keys
        dict = read(fid[iter])
        ConvertHDFtoVTP("E:/SecondApproach/TESTING_CPU/" * SimMetaDataWedge.SimulationName * "_" * lpad(iter,6,"0") * ".vtp", dict)
    end
    close(fid)
end
