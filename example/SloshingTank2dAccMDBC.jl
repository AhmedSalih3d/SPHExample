...
function LoadSloshingAccelerationSeries(AccelerationFile::String, ::Type{T}) where {T<:AbstractFloat}
    Times = T[]
    Values = SVector{2,T}[]

    for Row in CSV.File(AccelerationFile; delim=';')
        Time = T(getproperty(Row, Symbol("#Time")))
        LinearAccX = T(Row.LinearAccX)
        LinearAccZ = T(Row.LinearAccZ)
        push!(Times, Time)
        push!(Values, SVector{2,T}(LinearAccX, LinearAccZ))
    end
...
function ResolveFirstExistingPath(Candidates)
    ...
    error("Could not find required input file. Checked paths:\n$(join(Candidates, '\n'))")
end
...
let
    ...
    SimMetaData = UseMDBC ?
        SimulationMetaData{Dimensions,FloatType,NoShifting,NoKernelOutput,SimpleMDBC,StoreLog}(
        ...
        SimulationMetaData{Dimensions,FloatType,NoShifting,NoKernelOutput,NoMDBC,StoreLog}(
            SimulationName = SimulationName,
            SaveLocation = "W:/Simulations/$(SimulationName)",
            SimulationTime = 8.35,
            OutputTimes = 0.01,
    ...
    if DrivingMode == :motion
        ...
        AccelerationFile = ResolveFirstExistingPath([
            ...
            "W:/DualSPHysics_v5.4/examples/main/05_SloshingTank/CaseSloshingAccData.csv",
            "E:/DualSPHysics_v5.4/examples/main/05_SloshingTank/CaseSloshingAccData.csv",
        ])

        FluidAccelerationModel = LoadSloshingAccelerationSeries(AccelerationFile, FloatType)
    end
    ...
    RunSimulation(
        ...
        ParticleNormalsPath = UseMDBC ? GhostCSV : nothing,
